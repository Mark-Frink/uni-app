...
<script>
    ...
    // [364.54,2359.51,2330.86,2369.65],
    // [332.08,2273.4,2259.25,2333.54],
    // [274.81,2326.31,2270.1,2328.14],
    // [333.61,2347.18,2321.6,2351.44],
    ...
    // [2190.1,2148.35,2126.22,2190.1]],
		...
		mounted() {
			...
			// if(this.timeQuantum.length==100){
		...
		// 	},
			logstatrt() {
				uni.request({
				    url: 'static/json/riK.json', //仅为示例，并非真实接口地址。
				    success: (res) => {
							res.data.xAxis.map((val)=>{
								val.data = this.value.map(item=>item[0])
							});
							res.data.dataZoom.map((val)=>{
								val.startValue = this.value.length-30;
								val.endValue = this.value.length;
							});
							res.data.series.forEach((val,index)=>{
								switch (val.name){
									case "模拟数据":
										val.data= this.value.map(item=>{
